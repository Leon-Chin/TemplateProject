  ...
  return (
    <Row key={loan}>
      <Col span={24}>
        <Card
          title={
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <Tag color={loan.status === "PENDING" ? "gold" : "green"}>{loan ? loan.status : "--"}</Tag>
              <Tag color={loan.loan_type === "GIVEN" ? "volcano" : "geekblue"}>
                <span>{loan ? loan.loan_type : "--"}</span>
                <span style={{ marginLeft: 5 }}>{loan.loan_type === "GIVEN" ? <ArrowRightOutlined /> : <ArrowLeftOutlined />}</span>
              </Tag>
            </div>
          }
          bordered={false}
          ...
          extra={
            <div style={{ display: "flex", gap: 10 }}>
              <Popconfirm title="Delete the Loan" description="Are you sure to delete this loan?" onConfirm={handleDeleteLoan} okText="Yes" cancelText="No">
        ...
        >
          <div style={{ fontSize: 18 }}>Person: {loan ? loan.person : "--"}</div>
          <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
            <div style={{ fontSize: 18 }}>Loan Amount:</div>
            <span style={{ fontSize: 24, fontWeight: "bold" }}>{loan ? loan.amount : "--"}</span>
          </div>
          <div style={{ fontSize: 18 }}>Reason: {loan ? loan.reason : "--"}</div>
          <div style={{ display: "flex", gap: 30, justifyContent: "end" }}>
            <div style={{ fontSize: 16 }}>Date: {loan ? loan.date && formatDateToMalaysia(new Date(loan.date)) : "--"}</div>
          </div>
        </Card>
      </Col>
